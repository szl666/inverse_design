...
DimeNet = dimenet_plus_plus.DimeNetPlusPlusWrap(hidden_channels=256,
                                                ...
                                                use_pbc=True,
                                                num_targets=1)
model_CKPT = torch.load('checkpoint.pt')
...
def predict_ene(structure, write_poscar=False, step=None, file_path=None):
    absorbates = ['CO']
    ...
    atoms = [AseAtomsAdaptor.get_atoms(i['slab+CO']) for i in slabs.values()]
    data = atg.convert_all(atoms)
    for index, dataobject in enumerate(data):
        dataobject.sid = index
    databatch = Batch.from_data_list(data)
    pre_ene = DimeNet(databatch) * 0.7121978259612424 - 0.7213243512666709
    if write_poscar:
        poscar = Poscar(structure=structure)
        poscar.write_file(f'{file_path}/POSCAR_step{step}_{round(pre_ene,3)}')
    return min(pre_ene)
