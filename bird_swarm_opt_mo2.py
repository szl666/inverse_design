...
class Bird_swarm_opt:
    ...
    def search(self,
        ...
        if conti:
            x = torch.load("x7.pt")
            pop = x.size(0)
            fit = torch.load("fit7.pt")
            pFit = torch.load("pFit7.pt")
            pX = torch.load("pX7.pt")
            fMin = torch.load("fMin7.pt")
            bestIndex = torch.load("bestIndex7.pt")
            bestX = torch.load("bestX7.pt")
            b2 = torch.load("b27.pt")
        else:
            x = self.opt_matrix
            x.cuda()
    ...
    #     Update the individual's best fitness value and the global best one
            ...
            b2[index] = fMin
            torch.save(x, "x7.pt")
            torch.save(fit, "fit7.pt")
            torch.save(pFit, "pFit7.pt")
            torch.save(pX, "pX7.pt")
            torch.save(fMin, "fMin7.pt")
            torch.save(bestIndex, "bestIndex7.pt")
            torch.save(bestX, "bestX7.pt")
            torch.save(b2, "b27.pt")
