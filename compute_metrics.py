...
from eval_utils import (smact_validity, structure_validity, CompScaler,
                        get_fp_pdist, load_config, load_data,
                        get_crystals_list, prop_model_eval, compute_cov)
...
Percentiles = {
    ...
    # 'gaspy': np.array([0.39378, 0.46726, 0.512]),
    'gaspy': np.array([0.02504629, 0.048670, 0.0687]),
}
...
class RecEval(object):
    def __init__(self, pred_crys, gt_crys, stol=0.5, angle_tol=10, ltol=0.3):
        ...
        self.matcher = StructureMatcher(stol=stol,
                                        angle_tol=angle_tol,
                                        ltol=ltol)
    ...
    def get_match_rate_and_rms(self):
        def process_one(pred, gt, is_valid):
            if not is_valid:
                return None
            try:
                rms_dist = self.matcher.get_rms_dist(pred.structure,
                                                     gt.structure)
        ...
        for i in tqdm(range(len(self.preds))):
            rms_dists.append(
                process_one(self.preds[i], self.gts[i], validity[i]))
...
class GenEval(object):
    def __init__(self,
        ...
        if len(valid_crys) >= n_samples:
            sampled_indices = np.random.choice(len(valid_crys),
                                               n_samples,
                                               replace=False)
    ...
    def get_struct_diversity(self):
        return {
            'struct_div':
            get_fp_pdist([c.struct_fp for c in self.valid_samples])
        }
    ...
    def get_num_elem_wdist(self):
        pred_nelems = [
            len(set(c.structure.species)) for c in self.valid_samples
        ]
    ...
    def get_coverage(self):
        ...
        (cov_metrics_dict,
         combined_dist_dict) = compute_cov(self.crys,
                                           self.gt_crys,
...
class OptEval(object):
    ...
    def get_success_rate(self):
        ...
        if len(valid_crys) == 0:
            ...
            # print(props)
            # print(props.shape)
            # print(props.min(axis=0))
            props = abs(props + 0.67)
            best_props = props.min(axis=0)
            print(len(best_props))
            # np.save("best_props.npy", best_props)
            np.save("pred_props.npy", pred_props)
...
def get_crystal_array_list(file_path, batch_idx=0):
    ...
    crys_array_list = get_crystals_list(data['frac_coords'][batch_idx],
...
def main(args):
    ...
    if 'gen' in args.tasks:
        ...
        gen_evaluator = GenEval(gen_crys,
                                gt_crys,
