...
from cdvae.pl_data.dataset import TensorCrystDataset, CrystDataset
from cdvae.pl_data.datamodule import worker_init_fn

from torch_geometric.data import DataLoader

CompScaler = StandardScaler(means=np.array(CompScalerMeans),
...
def get_model_path(eval_model_name):
    import cdvae
    model_path = (Path(cdvae.__file__).parent / 'prop_models' /
                  eval_model_name)
    return model_path
...
def get_crystals_list(frac_coords, atom_types, lengths, angles, num_atoms):
    ...
    for batch_idx, num_atom in enumerate(num_atoms.tolist()):
        ...
        crystal_array_list.append({
            'frac_coords':
            cur_frac_coords.detach().cpu().numpy(),
            'atom_types':
            cur_atom_types.detach().cpu().numpy(),
            'lengths':
            cur_lengths.detach().cpu().numpy(),
            'angles':
            cur_angles.detach().cpu().numpy(),
        })
...
def smact_validity(comp, count, use_pauling_test=True, include_alloys=True):
    ...
    for ox_states in itertools.product(*ox_combos):
        ...
        if cn_e:
            ...
            if electroneg_OK:
                for ratio in cn_r:
                    compositions.append(tuple([elem_symbols, ox_states,
                                               ratio]))
...
def prop_model_eval(eval_model_name, crystal_array_list):
    ...
    dataset = TensorCrystDataset(crystal_array_list, cfg.data.niggli,
                                 cfg.data.primitive, cfg.data.graph_method,
                                 cfg.data.preprocess_workers,
                                 cfg.data.lattice_scale_method)
    # dataset = CrystDataset(crystal_array_list, cfg.data.niggli,
    #                        cfg.data.primitive, cfg.data.graph_method,
    #                        cfg.data.preprocess_workers,
    #                        cfg.data.lattice_scale_method)
...
def compute_cov(crys,
    ...
    cov_recall = np.mean(
        np.logical_and(struc_recall_dist <= struc_cutoff,
                       comp_recall_dist <= comp_cutoff))
    cov_precision = np.sum(
        np.logical_and(struc_precision_dist <= struc_cutoff,
