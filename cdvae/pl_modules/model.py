...
class CDVAE(BaseModule):
    ...
    def remove_nan_inf(self, x):
        nan_mask = torch.isnan(x)
        inf_mask = torch.isinf(x)
        x = torch.where(nan_mask | inf_mask, torch.zeros_like(x), x)
        return x
    ...
    def langevin_dynamics(self,
        ...
        print(self.hparams.sigma_begin)
        print(self.hparams.sigma_end)
        print(self.hparams.num_noise_level)
        print(self.hparams.type_sigma_begin)
        print(self.hparams.type_sigma_end)
        ...
        for sigma in tqdm(self.sigmas,
            ...
            for step in range(ld_kwargs.n_step_each):
                ...
                # print(angles)
                # print('----------------test_lv-------------------------')
                z = self.remove_nan_inf(z)
                cur_frac_coords = self.remove_nan_inf(cur_frac_coords)
                cur_atom_types = self.remove_nan_inf(cur_atom_types)
                num_atoms = self.remove_nan_inf(num_atoms)
                lengths = self.remove_nan_inf(lengths)
                angles = self.remove_nan_inf(angles)
    ...
    def forward(self, batch, teacher_forcing, training):
        ...
        pred_composition_probs = F.softmax(pred_composition_per_atom.detach(),
                                           dim=-1)
        pred_composition_probs = torch.tensor(
            np.nan_to_num(pred_composition_probs.cpu(),
                          nan=0,
                          posinf=10,
                          neginf=-10)).cuda()
        atom_type_probs = (
        ...
        try:
            atom_type_probs = torch.tensor(
                np.nan_to_num(atom_type_probs.cpu(),
                              nan=0,
                              posinf=10,
                              neginf=-10)).cuda()
