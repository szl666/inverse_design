...
from cdvae.common.data_utils import (get_pbc_distances, radius_graph_pbc,
                                     frac_to_cart_coords)
...
class GemNetT(torch.nn.Module):
    ...
    def __init__(
        ...
        AutomaticFit.reset(
        )  # make sure that queue is empty (avoid potential error)
        ...
        self.atom_latent_emb = nn.Linear(emb_size_atom + latent_dim,
                                         emb_size_atom)
        self.edge_emb = EdgeEmbedding(emb_size_atom,
                                      num_radial,
                                      emb_size_edge,
    ...
    def get_triplets(self, edge_index, num_atoms):
        ...
        value = torch.arange(idx_s.size(0),
                             device=idx_s.device,
                             dtype=idx_s.dtype)
    ...
    def reorder_symmetric_edges(self, edge_index, cell_offsets, neighbors,
                                edge_dist, edge_vector):
        """
        ...
        neighbors_new = 2 * torch.bincount(batch_edge,
                                           minlength=neighbors.size(0))
        ...
        cell_offsets_new = self.select_symmetric_edges(cell_offsets, mask,
                                                       edge_reorder_idx, True)
        edge_dist_new = self.select_symmetric_edges(edge_dist, mask,
                                                    edge_reorder_idx, False)
        edge_vector_new = self.select_symmetric_edges(edge_vector, mask,
                                                      edge_reorder_idx, True)
    ...
    def generate_interaction_graph(self, cart_coords, lengths, angles,
        ...
        (
            ...
            D_st,
            V_st,
        ) = self.reorder_symmetric_edges(edge_index, to_jimages, num_bonds,
                                         D_st, V_st)
        ...
        # print(block_sizes)
        # if len(block_sizes[:-1] + block_sizes[1:]) == 0:
        #     print('testtttttttttttttttttttttttttt')
        #     print(block_sizes[:-1])
        #     print(block_sizes[1:])
        #     print('testtttttttttttttttttttttttttt')
    ...
    def forward(self, z, frac_coords, atom_types, num_atoms, lengths, angles,
        ...
        E_t = scatter(E_t, batch, dim=0, dim_size=nMolecules,
                      reduce="mean")  # (nMolecules, num_targets)
