...
def repeat_blocks(
    ...
    # print(sizes)
    # print(sizes_nonzero)
    # print(torch.all(sizes_nonzero))
    # print('in utils ---------------------')
    if not torch.all(sizes_nonzero):
        # assert block_inc == 0  # Implementing this is not worth the effort
        sizes = torch.where(sizes == 0, torch.ones_like(sizes), sizes)
        # sizes = torch.masked_select(sizes, sizes_nonzero)
        if isinstance(repeats, torch.Tensor):
            repeats = torch.where(repeats == 0, torch.ones_like(repeats),
                                  repeats)
            # repeats = torch.masked_select(repeats, sizes_nonzero)
        if isinstance(repeat_inc, torch.Tensor):
            # repeat_inc = torch.masked_select(repeat_inc, sizes_nonzero)
            repeat_inc = torch.where(repeat_inc == 0,
                                     torch.ones_like(repeat_inc), repeat_inc)
    ...
    r1 = torch.repeat_interleave(torch.arange(len(sizes), device=sizes.device),
                                 repeats)
