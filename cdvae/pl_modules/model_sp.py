...
class CrystGNN_Supervise(BaseModule):
    ...
    def compute_stats(self, batch, preds, prefix):
        ...
        mae_origin = torch.mean(torch.abs(preds - batch.y))

        log_dict = {
            f'{prefix}_loss': loss,
            f'{prefix}_mae': mae,
            f'{prefix}_mae_origin': mae_origin
        }
        ...
        if self.hparams.data.prop == 'scaled_lattice':
            ...
            true_volumes = lengths_angles_to_volume(batch.lengths,
                                                    batch.angles)
...
class CDVAE(BaseModule):
    def __init__(self, *args, **kwargs) -> None:
        ...
        # self.fc_space_group = build_mlp(self.hparams.latent_dim,
        #                                 self.hparams.hidden_dim,
        #                                 self.hparams.fc_num_layers, 231)
    ...
    def decode_stats(self,
        ...
        if gt_num_atoms is not None:
            ...
            lengths_and_angles, lengths, angles = (self.predict_lattice(
                z, gt_num_atoms))
            # space_groups = self.predict_space_groups(z)
            ...
            num_atoms = torch.tensor([1 if i == 0 else i for i in num_atoms])
            num_atoms = num_atoms.cuda()
            lengths_and_angles, lengths, angles = (self.predict_lattice(
                z, num_atoms))
            # space_groups = self.predict_space_groups(z)
            composition_per_atom = self.predict_composition(z, num_atoms)
        # return num_atoms, lengths_and_angles, lengths, angles, composition_per_atom, space_groups
    ...
    def langevin_dynamics(self,
        ...
        # obtain key stats.
        # num_atoms, _, lengths, angles, composition_per_atom, space_groups = self.decode_stats(
        #     z, gt_num_atoms)
        ...
        if gt_atom_types is None:
            cur_atom_types = self.sample_composition(composition_per_atom,
                                                     num_atoms)
        else:
        ...
        if ld_kwargs.save_traj:
            output_dict.update(
                dict(all_frac_coords=torch.stack(all_frac_coords, dim=0),
    ...
    def get_space_groups(self, frac_coords, atom_types, num_atoms, lengths,
                         angles):
        frac_coords_c = frac_coords.clone()
        atom_types_c = atom_types.clone()
        lengths_c = lengths.clone()
        angles_c = angles.clone()
        location = 0
        space_groups = []
        for index, num_atom in enumerate(num_atoms):
            num_atom = int(num_atom)
            atom_type = atom_types_c.detach().cpu().numpy()[location:location +
                                                            num_atom]
            frac_coord = frac_coords_c.detach().cpu().numpy(
            )[location:location + num_atom]
            length = lengths_c.detach().cpu().numpy()[index]
            angle = angles_c.detach().cpu().numpy()[index]
            ...
            location += num_atom
        space_groups = torch.tensor(space_groups)
        return space_groups.cuda()
    ...
    def forward(self, batch, teacher_forcing, training):
        ...
        # print(batch.num_atoms.size())
        # print(batch.frac_coords)
        # print(batch.frac_coords.size())
        # print(batch.lengths)
        # print(batch.lengths.size())
        # print(batch.angles)
        # print(batch.angles.size())
        # print(batch.space_groups)
        # print(batch.space_groups.size())
        # print(batch.edge_index)
        # print(batch.edge_index.size())
        # print(batch.to_jimages)
        # print(batch.to_jimages.size())
        # print(batch.atom_types)
        # print(batch.atom_types.size())
        # print('-------------------')
        ...
        type_noise_level = torch.randint(0,
        ...
        pred_composition_probs = F.softmax(pred_composition_per_atom.detach(),
                                           dim=-1)
        atom_type_probs = (
        ...
        atom_type_probs = torch.tensor(
            np.nan_to_num(atom_type_probs.cpu(), nan=0, posinf=10,
                          neginf=-10)).cuda()
        rand_atom_types = torch.multinomial(atom_type_probs,
                                            num_samples=1).squeeze(1) + 1
        ...
        noisy_frac_coords = cart_to_frac_coords(cart_coords, pred_lengths,
                                                pred_angles, batch.num_atoms)

        pred_cart_coord_diff, pred_atom_types = self.decoder(
            z, noisy_frac_coords, rand_atom_types, batch.num_atoms,
            pred_lengths, pred_angles)
        # pred_atom_types_real = torch.argmax(pred_atom_types, dim=1) + 1
        # pred_space_groups = self.get_space_groups(pred_cart_coord_diff,
        #                                           pred_atom_types_real,
        #                                           batch.num_atoms,
        #                                           pred_lengths, pred_angles)
        # pred_space_groups = self.predict_space_groups(z)
        # compute loss.
        num_atom_loss = self.num_atom_loss(pred_num_atoms, batch)
        lattice_loss = self.lattice_loss(pred_lengths_and_angles, batch)
        # space_group_loss = self.space_group_loss(pred_space_groups, batch)**0.5
        # space_group_loss = self.space_group_loss(pred_space_groups, batch)
        composition_loss = self.composition_loss(pred_composition_per_atom,
        ...
        return {
            ...
            # 'space_group_loss': space_group_loss,
            'type_loss': type_loss,
            'kld_loss': kld_loss,
            ...
            # 'pred_space_groups': pred_space_groups,
    ...
    def generate_rand_init(self, pred_composition_per_atom, pred_lengths,
        ...
        rand_frac_coords = torch.rand(num_atoms.sum(),
                                      3,
                                      device=num_atoms.device)
        pred_composition_per_atom = F.softmax(pred_composition_per_atom,
                                              dim=-1)
        rand_atom_types = self.sample_composition(pred_composition_per_atom,
                                                  num_atoms)
    ...
    def sample_composition(self, composition_prob, num_atoms):
        ...
        """
        # print('num_atoms')
        # print(num_atoms.tolist())
        batch = torch.arange(
            len(num_atoms),
            device=num_atoms.device).repeat_interleave(num_atoms)
        # assert composition_prob.size(0) == num_atoms.sum() == batch.size(0)
        composition_prob = scatter(composition_prob,
                                   index=batch,
                                   dim=0,
                                   reduce='mean')
        # print('composition_prob')
        # print(composition_prob.tolist())

        all_sampled_comp = []

        for comp_prob, num_atom in zip(list(composition_prob),
                                       list(num_atoms)):
            comp_num = torch.round(comp_prob *
                                   num_atom) if num_atom > 1 else torch.ceil(
                                       comp_prob * num_atom)
            # print('comp_num')
            # print(comp_num.tolist())
            atom_type = torch.nonzero(comp_num, as_tuple=True)[0] + 1
            # print('atom_type')
            # print(atom_type.tolist())
            atom_num = comp_num[atom_type - 1].long()
            # print('atom_num')
            # print(atom_num.tolist())
            ...
            if sampled_comp.size(0) < num_atom:
                ...
                left_comp = torch.multinomial(left_comp_prob,
    ...
    # def predict_space_groups(self, z):
    #     return self.fc_space_group(z)
    ...
    def predict_lattice(self, z, num_atoms):
        ...
        pred_angles = scaled_preds[:, 3:]
        if self.hparams.data.lattice_scale_method == 'scale_length':
            pred_lengths = pred_lengths * num_atoms.view(-1,
                                                         1).float()**(1 / 3)
    ...
    def space_group_loss(self, pred_space_groups, batch):
        real_space_groups = torch.LongTensor(np.array(
            batch.space_groups)).cuda()
        return F.cross_entropy(pred_space_groups, real_space_groups)
    ...
    def coord_loss(self, pred_cart_coord_diff, noisy_frac_coords,
        ...
        target_cart_coords = frac_to_cart_coords(batch.frac_coords,
                                                 batch.lengths, batch.angles,
                                                 batch.num_atoms)
        _, target_cart_coord_diff = min_distance_sqr_pbc(target_cart_coords,
                                                         noisy_cart_coords,
                                                         batch.lengths,
                                                         batch.angles,
                                                         batch.num_atoms,
    ...
    def type_loss(self, pred_atom_types, target_atom_types,
                  used_type_sigmas_per_atom, batch):
        target_atom_types = target_atom_types - 1
        loss = F.cross_entropy(pred_atom_types,
                               target_atom_types,
                               reduction='none')
    ...
    def compute_stats(self, batch, outputs, prefix):
        ...
        log_dict = {
            ...
            f'{prefix}_space_group_loss': space_group_loss,
        ...
        if prefix != 'train':
            ...
            print(pred_num_atoms)
            print(batch.num_atoms)
            num_atom_accuracy = (pred_num_atoms
                                 == batch.num_atoms).sum() / batch.num_graphs
            pred_space_groups = outputs['pred_space_groups'].argmax(dim=-1)
            real_space_groups = torch.LongTensor(np.array(
                batch.space_groups)).cuda()
            space_group_accuracy = (pred_space_groups == real_space_groups
            ...
            log_dict.update({
                ...
                f'{prefix}_space_group_accuracy': space_group_accuracy,
                f'{prefix}_lengths_mard': lengths_mard,
                f'{prefix}_angles_mae': angles_mae,
                f'{prefix}_volumes_mard': volumes_mard,
