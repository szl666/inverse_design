...
CrystalNN = local_env.CrystalNN(distance_cutoffs=None,
                                x_diff_weight=-1,
...
def build_crystal_graph(crystal, graph_method='crystalnn'):
    ...
    space_groups = np.array(float(crystal.get_space_group_info()[1]))
    ...
    return frac_coords, atom_types, lengths, angles, space_groups, edge_indices, to_jimages, num_atoms
...
def lattice_params_to_matrix_torch(lengths, angles):
    ...
    vector_b = torch.stack([
        -lengths[:, 1] * sins[:, 0] * torch.cos(gamma_star), lengths[:, 1] *
        sins[:, 0] * torch.sin(gamma_star), lengths[:, 1] * coses[:, 0]
    ],
                           dim=1)
    vector_c = torch.stack([
        torch.zeros(lengths.size(0), device=lengths.device),
        torch.zeros(lengths.size(0), device=lengths.device), lengths[:, 2]
    ],
                           dim=1)
...
def radius_graph_pbc_wrapper(data, radius, max_num_neighbors_threshold,
                             device):
    cart_coords = frac_to_cart_coords(data.frac_coords, data.lengths,
                                      data.angles, data.num_atoms)
    return radius_graph_pbc(cart_coords, data.lengths, data.angles,
                            data.num_atoms, radius,
                            max_num_neighbors_threshold, device)


def radius_graph_pbc(cart_coords,
                     lengths,
                     angles,
                     num_atoms,
                     radius,
    ...
    index_offset = (torch.cumsum(num_atoms_per_image, dim=0) -
                    num_atoms_per_image)

    index_offset_expand = torch.repeat_interleave(index_offset,
    ...
    index_sqr_offset = (torch.cumsum(num_atoms_per_image_sqr, dim=0) -
                        num_atoms_per_image_sqr)
    index_sqr_offset = torch.repeat_interleave(index_sqr_offset,
                                               num_atoms_per_image_sqr)
    atom_count_sqr = (torch.arange(num_atom_pairs, device=device) -
                      index_sqr_offset)

    # Compute the indices for the pairs of atoms (using division and mod)
    # If the systems get too large this apporach could run into numerical precision issues
    index1 = ((atom_count_sqr //
               num_atoms_per_image_expand)).long() + index_offset_expand
    index2 = (atom_count_sqr %
              num_atoms_per_image_expand).long() + index_offset_expand
    ...
    unit_cell_per_atom = unit_cell.view(1, num_cells,
                                        3).repeat(len(index2), 1, 1)
    unit_cell = torch.transpose(unit_cell, 0, 1)
    unit_cell_batch = unit_cell.view(1, 3,
                                     num_cells).expand(batch_size, -1, -1)
    ...
    pbc_offsets_per_atom = torch.repeat_interleave(pbc_offsets,
                                                   num_atoms_per_image_sqr,
                                                   dim=0)
    ...
    if topk_per_pair is not None:
        ...
        assert atom_distance_sqr_sort_index.size() == (num_atom_pairs,
                                                       num_cells)
        atom_distance_sqr_sort_index = (
            atom_distance_sqr_sort_index +
            torch.arange(num_atom_pairs, device=device)[:, None] *
            num_cells).view(-1)
    ...
    unit_cell = torch.masked_select(unit_cell_per_atom.view(-1, 3),
    ...
    num_neighbors_image = (_num_neighbors[_natoms[1:]] -
                           _num_neighbors[_natoms[:-1]])
    ...
    if (max_num_neighbors <= max_num_neighbors_threshold
        ...
        if topk_per_pair is None:
            return torch.stack(
                (index2, index1)), unit_cell, num_neighbors_image
        else:
            return torch.stack(
                (index2, index1)), unit_cell, num_neighbors_image, topk_mask

    atom_distance_sqr = torch.masked_select(atom_distance_sqr, mask)

    # Create a tensor of size [num_atoms, max_num_neighbors] to sort the distances of the neighbors.
    # Fill with values greater than radius*radius so we can easily remove unused distances later.
    distance_sort = torch.zeros(len(cart_coords) * max_num_neighbors,
                                device=device).fill_(radius * radius + 1.0)

    # Create an index map to map distances from atom_distance_sqr to distance_sort
    index_neighbor_offset = torch.cumsum(num_neighbors, dim=0) - num_neighbors
    index_neighbor_offset_expand = torch.repeat_interleave(
        index_neighbor_offset, num_neighbors)
    index_sort_map = (index1 * max_num_neighbors +
...
def min_distance_sqr_pbc(cart_coords1,
                         cart_coords2,
                         lengths,
                         angles,
                         num_atoms,
    ...
    if return_vector:
        ...
        min_atom_distance_vector = torch.gather(atom_distance_vector, 2,
                                                min_indices).squeeze(-1)
...
class StandardScalerTorch(object):
    ...
    def copy(self):
        return StandardScalerTorch(means=self.means.clone().detach(),
...
def preprocess(input_file, num_workers, niggli, primitive, graph_method,
    ...
    def process_one(row, niggli, primitive, graph_method, prop_list):
        crystal_str = row['cif']
        crystal = build_crystal(crystal_str,
                                niggli=niggli,
                                primitive=primitive)
    ...
    unordered_results = p_umap(process_one,
                               ...
                               [niggli] * len(df), [primitive] * len(df),
                               [graph_method] * len(df), [prop_list] * len(df),
...
def batch_accuracy_precision_recall(pred_edge_probs, edge_overlap_mask,
                                    num_bonds):
    if (pred_edge_probs is None and edge_overlap_mask is None
    ...
    for num_bond in num_bonds.tolist():
        pred_edge = pred_edges.narrow(0, start_idx,
                                      num_bond).detach().cpu().numpy()
        target_edge = target_edges.narrow(0, start_idx,
                                          num_bond).detach().cpu().numpy()
...
class StandardScaler:
    ...
    def fit(self, X):
        ...
        self.means = np.where(np.isnan(self.means), np.zeros(self.means.shape),
                              self.means)
